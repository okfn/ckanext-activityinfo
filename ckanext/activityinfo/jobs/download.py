...
def download_activityinfo_resource(resource_id: str, user: str) -> None:
    ...
    try:
        ...
        _update_resource_status(context, resource_id, 'exporting', 0)
        ...
        while elapsed < max_wait:
            ...
            # Update progress
            _update_resource_status(context, resource_id, 'exporting', percent)
            ...
            if state == 'completed':
                ...
                _update_resource_status(context, resource_id, 'downloading', 100)
                ...
                _update_resource_with_file(context, resource_id, file_data, filename, format_type)
        ...
        log.error(f"ActivityInfo Job: Failed for resource {resource_id}: {e}")
        _update_resource_status(context, resource_id, 'error', 0, str(e))
        raise
...
def _update_resource_status(context: dict, resource_id: str, status: str,
    ...
    try:
        ...
        log.error(f"ActivityInfo Job: Failed to update status for {resource_id}: {e}")
    ...
    try:
        with open(tmp_path, 'rb') as f:
            mime_type = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' \
                if format_type == 'xlsx' else 'text/csv'
